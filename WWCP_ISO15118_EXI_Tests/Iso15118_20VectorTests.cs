...
namespace cloud.charging.open.protocols.ISO15118.EXI.Tests
{
    ...
    public class Iso15118_20VectorTests
    {
        ...
        // WPT and ACDP are the two sets where this codec deliberately stopped reproducing cbexigen on
        // 2026-08-08 — the document-element numbering for ACDP, the mid-run particle grammar for WPT,
        // both following ISO's schema and the EXI specification where cbexigen departs from them. Most
        // of each corpus is still cbV2G's output and still matches; the six vectors that are not say so
        // in their file's `deviatesFromReferenceEncoder`, and are verified through EXIficient instead.
        // Hence "_Matches_Corpus" rather than "_Matches_CbV2G": the file is the authority, not cbV2G.

        [TestCaseSource(nameof(WptVectors))]
        public void WPT_Matches_Corpus(Vec vector) =>
            AssertMatches(vector, Iso15118_20WptFixtures.TryEncode);

        [TestCaseSource(nameof(AcdpVectors))]
        public void ACDP_Matches_Corpus(Vec vector) =>
        ...
        private static void AssertMatches(Vec vector, TryEncodeFn tryEncode)
        {
            ...
            if (!actual.AsSpan().SequenceEqual(expected))
                Assert.Fail($"{vector.Name}: generated bytes diverge from the corpus.\n" +

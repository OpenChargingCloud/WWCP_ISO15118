...
namespace cloud.charging.open.protocols.ISO15118.EXI.Tests
{
    ...
    public class V2GTPFrameTests
    {
        ...
        public void Header_Roundtrip()
        {
            ...
            var buf = new byte[V2GTPCodec.HeaderSize];
            V2GTPCodec.WriteHeader(buf, V2GTPCodec.PayloadType_AppProtocol, 42);   // 0x8001 (SAP / -2 EXI payload id)
            ...
            Assert.That(V2GTPCodec.TryReadHeader(buf, out var pt, out var plen), Is.True);
            Assert.That(pt,   Is.EqualTo(V2GTPCodec.PayloadType_AppProtocol));
        ...
        public void Header_Rejects_WrongVersion()
        {
            ...
            Assert.That(V2GTPCodec.TryReadHeader(buf, out _, out _), Is.False);
        }
        ...
        public void ThePayloadIdsAreTheOnesTheStandardGives()
        {
            Assert.Multiple(() =>
            {
                ...
                Assert.That(V2GTPCodec.PayloadType_DinIso2Main, Is.EqualTo((ushort) 0x8001),
                            "the handshake and -2 share an id — SAP has none of its own");
                Assert.That(V2GTPCodec.PayloadType_Iso20Main,   Is.EqualTo((ushort) 0x8002));
                Assert.That(V2GTPCodec.PayloadType_Iso20AC,     Is.EqualTo((ushort) 0x8003));
                Assert.That(V2GTPCodec.PayloadType_Iso20DC,     Is.EqualTo((ushort) 0x8004));
                Assert.That(V2GTPCodec.PayloadType_Iso20ACDP,   Is.EqualTo((ushort) 0x8005));
                Assert.That(V2GTPCodec.PayloadType_Iso20WPT,    Is.EqualTo((ushort) 0x8006));
            });
        }
    }
}
